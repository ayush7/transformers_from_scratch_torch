...
class SelfAttention(nn.Module):
    ...
    def forward(self, values, keys, query, mask):
        ...
        # Split the embeddings in self.heads
        ...
        if mask is None:
            energy.masked_fill(mask==0, float("-1e28"))
...
class DecoderBlock(nn.Module):
    def __init__(self, embed_size, heads, forwardexpansion, dropout, device):
        super(DecoderBlock, self).__init__()
        self.attention = SelfAttention(embed_size, heads)
        self.norm = nn.LayerNorm(embed_size)
        self.transformer_block = TransformerBlock(embed_size, heads, dropout, forwardexpansion)
        self.dropout = nn.Dropout(dropout)
...
class Decoder(nn.Module):
    def __init__(self, target_voceb_size,
                 embed_size,
                 num_layers,
                 heads,
                 forward_expansion,
        ...
        self.device = device
        self.word_embedding = nn.Embedding(target_voceb_size, embed_size)
        self.position_embedding = nn.Embedding(max_length, embed_size)
        ...
        self.fc_out = nn.Linear(embed_size, target_voceb_size)
        self.dropout = nn.Dropout(dropout)
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.decoder = Decoder(
            tgt_vocab_size, embed_size,num_layers, heads, forward_expansions,dropout, device,max_length
        )
